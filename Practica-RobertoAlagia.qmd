...
```{r}
ap_matrix <- matrix(df_madrid_clean)
pr_ap<-prcomp(df_madrid_clean,center = TRUE, scale. = TRUE)
```

```{r}
buscar_pisos_similares <- function(datos, Accommodates, Bathrooms, Bedrooms, Beds, Price, Guests.Included, Extra.People, Review.Scores.Rating, Latitude, Longitude, Square.Meters) {
  # Calcular la distancia euclidiana entre el piso dado y todos los pisos en el conjunto de datos
  distancias <- rowSums((t_ap[rep(1, times = nrow(pr_ap$x)), ]-pr_ap$x)^2)
  
  # Ordenar los pisos según la distancia y seleccionar los 5 más cercanos
  pisos_similares <- datos[order(distancias), ][1:5, ]
  
  # Devolver los 5 pisos más similares
  return(pisos_similares)
}
```

```{r}
busqueda <- buscar_pisos_similares(df_madrid_clean,5,2,3,3,120,3,25,80,40.41682,-3.708097,60)
